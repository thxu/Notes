...
### 7.1 背景
长期会话与频繁的工具调用会让 `messages` 快速膨胀，既增加成本也可能超过模型的令牌限制。工具返回（如文件内容、命令输出）尤其容易占用大量上下文。s06 的目标是以最低的信息损失控制上下文规模，使 Agent 能够“无限”运行。

### 7.2 当前问题
简单地删除早期消息会丢失关键的工具输出或参考资料，导致代理重复读取、效率下降或丢失可审计的历史记录。

### 7.3 总体解决
s06 采用三层压缩管线：
- Layer 1 `micro_compact`：每轮静默精简非关键工具结果（保留最近若干）。
- Layer 2 `auto_compact`：当 token 估算超过阈值时，将完整对话持久化并调用 LLM 生成摘要，替换历史消息为压缩后的摘要块。
- Layer 3 `manual compact`：通过 `compact` 工具手动触发第二层压缩。

文件位置：[agents/s06_context_compact.py](agents/s06_context_compact.py#L1)

### 7.4 关键常量与设计
- **THRESHOLD** = `50000`：触发自动压缩的 token 估算阈值。
- **KEEP_RECENT** = `3`：`micro_compact` 保留最近多少个工具结果。
- **TRANSCRIPT_DIR** = `WORKDIR / ".transcripts"`：自动压缩时写入完整 transcript 的目录。
- **PRESERVE_RESULT_TOOLS** = `{"read_file"}`：不被 micro_compact 静默替换的工具（常为引用型资料）。

### 7.5 Layer 1：micro_compact（每轮静默执行）
- 遍历 `messages`，收集所有 `type=="tool_result"` 的块，并结合先前的 assistant 中的 `tool_use` 块构建 `tool_use_id -> tool_name` 映射。
- 对于超过 `KEEP_RECENT` 的旧 `tool_result`：若结果是字符串且长度 > 100，并且 `tool_name` 不在 `PRESERVE_RESULT_TOOLS` 中，就将其内容替换为占位符，例如：`[Previous: used {tool_name}]`。
...
此层的好处是保留“曾经使用过某工具并得到过某类结果”的语义，同时避免把大段文本长期保留在上下文中。

### 7.6 Layer 2：auto_compact（阈值触发的摘要替换）
- 使用 `estimate_tokens(messages)`（仓库中实现为 `len(str(messages)) // 4` 的近似）判断是否超过 `THRESHOLD`。
- 若超过：将完整 `messages` 写入 `TRANSCRIPT_DIR`（每条消息一行的 JSONL），并将最近约 80k 字符的对话片段截取为 `conversation_text = json.dumps(messages, default=str)[-80000:]` 发给 LLM 请求摘要（`max_tokens=2000`）。
- 从模型响应中提取文本块作为 `summary`，然后把全部 `messages` 替换为一条用户消息，内容像：

```text
[Conversation compressed. Transcript: .transcripts/transcript_{ts}.jsonl]
...
messages[:] = [{"role":"user","content": f"[Conversation compressed. Transcript: {transcript_path}]\n\n{summary}"}]
```

自动压缩通过持久化完整 transcript 保留审计能力，同时把会话上下文缩成可供后续决策使用的精炼摘要。

### 7.7 Layer 3：manual compact（手动触发）
- 代理暴露一个名为 `compact` 的工具；当模型以 `tool_use` 方式调用 `compact` 时，`agent_loop` 会把 `manual_compact` 标志置为真。工具完成后，循环会调用 `auto_compact(messages)` 并返回，从而立即完成摘要压缩。

### 7.8 在 `agent_loop` 中的集成要点
- 每轮开始先调用 `micro_compact(messages)`；随后若 `estimate_tokens(messages) > THRESHOLD` 则执行 `auto_compact(messages)`。
- 调用模型时把 `tools=TOOLS` 传入；若 `response.stop_reason == "tool_use"`，对每个 `tool_use` block 调用 handler 并把结果以 `{"type":"tool_result", ...}` 的块追加到 `messages`。
- 若检测到 `compact` 被触发（manual compact），则在工具返回后立即执行 `auto_compact` 并返回。代码要点见 `agents/s06_context_compact.py`。

### 7.9 设计权衡与实践建议
- 将 `read_file` 等引用类工具列入 `PRESERVE_RESULT_TOOLS` 可避免重复读取重要参考资料，提升效率。
- 自动摘要保留了可审计的 JSONL transcript（`.transcripts/`），便于离线检索与回溯。
- 这套方案在教学与原型中平衡了信息保留与上下文成本；生产环境可引入更精细的令牌估算和基于 embedding 的压缩策略。

### 7.10 关键洞察
- **可持续性**：三层压缩使 Agent 能长期运行而不被上下文限制吞没。
- **审计性**：写入 transcript 保证历史可追溯。
- **实用性**：优先保留引用资料（如文件内容），静默替换大量临时输出。
